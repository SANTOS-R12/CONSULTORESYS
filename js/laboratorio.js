            ...
            laboratorios.forEach(laboratorio => {
                template += `
                    <tr labid="${laboratorio.id}" labnombre="${laboratorio.nombre}" labavatar="${laboratorio.avatar}">
                        ...
                        <td>
                            <button class="avatar btn btn-info btn-sm" title="Cambiar logo de marca" type="button" data-toggle="modal" data-target="#cambio_logo">
                                <i class="fas fa-image"></i>
                            </button>
                            <button class="editar btn btn-success btn-sm" title="Editar marca" type="button" data-toggle="modal" data-target="#crearlaboratorio">
                                <i class="fas fa-pencil-alt"></i>
                            </button>
                            <button class="borrar btn btn-danger btn-sm" title="Borrar marca">
            ...
            if (result.value) {
                $.post('../controlador/LaboratorioController.php', {id, funcion}, (response) => {
                    ...
                    if (response == 'borrado') {
                        swalWithBootstrapButtons.fire(
                            'Eliminado!',
                            'La licitacion '+nombre+' fue borrado.',
                            'success'
                        )
                        buscar_lab();
                    }else{
                        swalWithBootstrapButtons.fire(
                            'No se pudo borrar!',
                            'La licitacion '+nombre+' no fue borrado porque esta siendo utilizado en un producto.',
                            'error'
                        )
                    }
                });
              ...
              swalWithBootstrapButtons.fire(
                'Cancelado',
                'La licitacion '+nombre+' no fue borrado',
                'error'
              )
            }
          })
